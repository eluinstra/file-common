...
public class PGPEncryptionExample
{
	...
	/**
	 * Create an encrypted data blob using an AES-256 session key and the passed in public key.
	 *
	...
	public static byte[] createEncryptedData(PGPPublicKey encryptionKey, byte[] data) throws PGPException, IOException
	{
		PGPEncryptedDataGenerator encGen = new PGPEncryptedDataGenerator(
				new JcePGPDataEncryptorBuilder(SymmetricKeyAlgorithmTags.AES_256).setWithIntegrityPacket(true).setSecureRandom(new SecureRandom()).setProvider("BC"));
	...
	/**
	 * Extract the plain text data from the passed in encoding of PGP encrypted data. The routine assumes the passed in private key is the one that matches the
	 * first encrypted data object in the encoding.
	 *
	 ...
	 */
	public static byte[] extractPlainTextData(PGPPrivateKey privateKey, byte[] pgpEncryptedData) throws PGPException, IOException
	{
